...
Andiamo quindi a scalare la nostra tabella:
```{r}
tabellaScalata <- scale(tabella)
tabellaScalata
```

Una volta effettuato lo scalamento è possibile passare alla sezione riguardante la distanza e la similarità fra due regioni.
...
```{r}
dist(tabellaScalata, method="euclidean", diag=FALSE, upper=FALSE)
```
