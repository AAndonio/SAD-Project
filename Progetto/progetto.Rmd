...
Fi <- cumsum(table(cut(famiglia, breaks=fempcont_classi, right =FALSE)))/length(famiglia)
Fi
```
...
```{r}
Fi <- c(0, Fi)
plot(fempcont_classi, Fi, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
...
```{r}
Fi <- cumsum(table(cut(lavoro, breaks=fempcont_classi, right =FALSE)))/length(lavoro)
Fi <- c(0, Fi)
plot(fempcont_classi, Fi, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
axis(2, format(Fi, digits=2))
...
```{r}
Fi <- cumsum(table(cut(asilo, breaks=fempcont_classi, right =FALSE)))/length(asilo)
Fi <- c(0, Fi)
plot(fempcont_classi, Fi, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
axis(2, format(Fi, digits=2))
...
```{r}
fempcont_classi <- c(0,2,4,6,8,10)
Fi <- cumsum(table(cut(studio, breaks=fempcont_classi, right =FALSE)))/length(studio)
Fi <- c(0, Fi)
plot(fempcont_classi, Fi, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
axis(2, format(Fi, digits=2))
...
```{r}
Fi <- cumsum(table(cut(altro, breaks=fempcont_classi, right =FALSE)))/length(altro)
Fi <- c(0, Fi)
plot(fempcont_classi, Fi, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
axis(2, format(Fi, digits=2))
...
Supponiamo di avere a disposizione un insieme $x_{1}, x_{2}, ... ,x_{n}$ di n valori, chiamato *campione di ampiezza n*. La **media campionaria** è la media aritmetica di questi valori.
...
Sia X una variabile quantitativa e siano $z_{1}$, $z_{2}$, ..., $z_{k}$ le modalità distinte da essa assunte con $z_{1}$ < $z_{2}$ < ... < $z_{k}$
