...
Se $\gamma_{1}\gt0$ allora l'asimetria è positiva (la coda di destra è più allungata);
Se $\gamma_{1}\lt0$ l'asimmetria è negativa (la coda di sinistra è più allungata).
...
```{r}
plot(stime, residuistandard, main = "Residui standard rispetto ai valori stimati", xlab="Valori stimati", ylab="Residui standard", pch=5, col="red")
abline(h=0, col="blue", lty=2)
```
...
```{r}
plot(stimemult,residuimultstandard, main="Residui standard rispetto ai valori stimati", ylab="Residui standard", xlab="Valori Stimati", pch=5, col="red")
abline(h=0, col="blue", lty=2)
```
...
Com'è possibile notare, il coefficiente di determinazione ottenuto col modello di regressione non lineare polinomia dal valore di 0.7709 è maggiore di quello di regressione lineare dal valore di 0.6252, risultando quindi più ottimale.

Andiamo adesso ad aggiungere la curva appena descritta allo scatterplot: 

```{r}
plot(lavoro, asilo, main="Scatterplot con curva", xlab="Lavoro", ylab="Asilo", col = "red")
...
La metrica più diffusa e familiare e quella **euclidea**, definita come segue:
...
In generale, una funzione a valori reali $s_{ij} = s(X_i, X_j)$ è detta misura di similarità se e soltanto se essa sosddisfa le seguenti condizioni:
...
Per utilizzare il metodo k-means in R bisogna utilizzare la funzione *kmeans(X, centers, iter.max=N, nstart=M)*. Per la nostra analisi utilizzeremo un valore di centers pari a 4, un numero di iterazioni pari a 25 e un valore di nstart uguale a 10.
...
- $C_2 = {Puglia, Basilicata, Calabria, Sicilia, Sardegna}$
- $C_3 = {Molise}$
- $C_4 = {Piemonte, Valle d'Aosta, Liguria, Lombardia, Trentino-Alto Adige, Veneto, Friuli-Venezia Giulia, Emilia-Romagna, Umbria, Marche, Abruzzo}$
...
Possiamo notare che la misura di non omogeneità totale è pari a 7067.801, mentre la somma delle misure di non omogeneità totale interne è uguale a 1039.17, la misura di non omogeneità tra cluster diversi è 6028.62 e il rapporto fra $\frac{trB}{trT}$ risulta essere pari a 0.853, un valore decisamente ottimo considerando il numero di centri utilizzato.
...
Per scegliere una buona partizione del dendrogramma, consideriamo un grafico detto **screenplot**, sul quale asse delle ordinate vengono posti i numeri di gruppi ottenibili con il meotodo gerarchico e sull'asse ascisse vengono le distanze a cui avvengono le successive aggregazioni fra gruppi. 
...
Con la suddivisione in tre cluster il rapporto fra trB/trHI è pari a 0.4232935 un valore che, seppur più alto di quello a due cluster risulta comunque essere basso.
...
La suddivisione in quattro cluster, suggeritaci durante l'analisi con i metodi non gerarchici, ci ha portato al risultato migliore fra le tre suddivisioni con un rapporto fra trB/trHI pari a 0.7699938, che ricade nel range di valori accettabili.
...
Come fatto in precedenza, procediamo alla stampa dello screenplot:
```{r}
plot(c(0, hlc$height), seq(20, 1), type = "b", main = "Screenplot", xlab = "Distanza di aggregazione", ylab = "Numero di cluster", col = "red")
```
...
Procediamo adesso all'analisi utilizzando le misure di non omogeneità statistiche, in quanto, anche in precedenza sono risultate essere più precise rispetto allo screenplot.
...
Con la suddivisione in tre cluster invece il rapporto tra $\frac{trB}{trT}$ ha raggiunto un risultato di sufficienza ed è possibile tenerlo in considerazione.
...
Con quest'ultima suddivisione, suggeritaci dal metodo k-means fatto in precedenza, il rapporto fra $\frac{trB}{trT}$ raggiunge il risultato più alto fra tutti quelli visti con i metodi gerarchici e risulta essere un ottimo risultato dato il numero di cluster scelti. Da notare che il valore è pressochè identico a quello trovato con il metodo k-means. 
...
Come fatto in precedenza, procediamo alla stampa dello screenplot:
```{r}
plot(c(0, hlm$height), seq(20, 1), type = "b", main = "Screenplot", xlab = "Distanza di aggregazione", ylab = "Numero di cluster", col = "red")
```
...
Con quest'ultima suddivisione, suggeritaci dal metodo k-means fatto in precedenza, il rapporto fra $\frac{trB}{trT}$ raggiunge il risultato più alto fra i precedenti ed è in linea con quello ottenuto con il metodo del legame completo e risulta essere un ottimo risultato dato il numero di cluster scelti. Da notare che il valore è pressochè identico a quello trovato con il metodo k-means. 
...
```{r}
plot(hlcentroid, hang = -1, xlab = "Metodo gerarchico agglomerativo", sub="del centroide")
...
Con quest'ultima suddivisione, suggeritaci dal metodo k-means fatto in precedenza, il rapporto fra $\frac{trB}{trT}$ raggiunge il risultato più alto fra i precedenti ed è poco inferiore a quello ottenuto con i due metodi precedenti e risulta essere un ottimo risultato dato il numero di cluster scelti.
...
```{r}
plot(hlmedian, hang = -1, xlab = "Metodo gerarchico agglomerativo", sub="della mediana")
