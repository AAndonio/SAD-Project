...
```{r}
fempcont_classi <- c(0,15,25,30,35,40,45,50,55,60,70)

Ffamiglia <- cumsum(table(cut(famiglia, breaks=fempcont_classi, right =FALSE)))/length(famiglia)
Ffamiglia
```
...
plot(fempcont_classi, Ffamiglia, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
...
```{r}
Flavoro <- cumsum(table(cut(lavoro, breaks=fempcont_classi, right =FALSE)))/length(lavoro)
Flavoro <- c(0, Flavoro)
plot(fempcont_classi, Flavoro, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
...
```{r}
Fasilo <- cumsum(table(cut(asilo, breaks=fempcont_classi, right =FALSE)))/length(asilo)
Fasilo <- c(0, Fasilo)
plot(fempcont_classi, Fasilo, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
...
```{r}
fempcont_classi1 <- c(0,2,4,6,8,10)
Fstudio <- cumsum(table(cut(studio, breaks=fempcont_classi1, right =FALSE)))/length(studio)
Fstudio <- c(0, Fstudio)
plot(fempcont_classi1, Fstudio, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi1)
axis(2, format(Fstudio, digits=2))
box()
```

```{r}
Faltro <- cumsum(table(cut(altro, breaks=fempcont_classi1, right =FALSE)))/length(altro)
Faltro <- c(0, Faltro)
plot(fempcont_classi1, Faltro, type = "b", axes = FALSE, main = "Funzione di distribuzione empirica continua", ylab ="Funzione", xlab = "Classi", col = "red")
axis(1, fempcont_classi)
...
```{r}
par(mfrow = c(2,2))
plot(fempcont_classi, Flavoro, type="b", main="Funzione di distribuzione empirica discreta di lavoro", col="red")
abline(h=0.5, lty=2, col="blue")
plot(fempcont_classi, Ffamiglia, type="b", main="Funzione di distribuzione empirica discreta di famiglia", col="red")
abline(h=0.5, lty=2, col="blue")
plot(fempcont_classi, Fasilo, type="b", main="Funzione di distribuzione empirica discreta di asilo", col="red")
abline(h=0.5, lty=2, col="blue")
plot(fempcont_classi1, Fstudio, type="b", main="Funzione di distribuzione empirica discreta di studio", col="red")
abline(h=0.5, lty=2, col="blue")
plot(fempcont_classi1, Faltro, type="b", main="Funzione di distribuzione empirica discreta di altro", col="red")
abline(h=0.5, lty=2, col="blue")
```
...
Per il calcolo dei quantili R ha a disposizione la funzione *quantile(v, probs = , type = )*. Oltre al vettore, la funzione prende in input anche due ulteriori parametri *probs* che rappresenta il vettore delle probabilità e *type* a cui è possibile assegnare valori da 1 a 9 e indica l'algoritmo da utilzzare per il calcolo dei quantili. Infatti, R possiede per 9 algoritmi per il calcolo dei quantili e quello di default è il 7, basato su tecniche di interpolazione tra i punti. 